...
The Business Basic trial's expiration read `9/22/2026`, resolving the track's dated item at last. Microsoft Entra ID P1 remained active and paid, unrelated to either trial.
...
**This lab got two things wrong about itself, and what caught both was reading live state at the start of a step instead of carrying the previous step's Final State forward.** Step Eight's Final State was accurate when it was written and inaccurate within a day, because `admin@brindeck.com` removed `John Smith` from `Company Announcements` at 4:03:32 PM on 9/7 after the step had closed, taking his license with him. Nothing noticed until Step Ten opened with its own baseline read and found the tenant no longer matched the document. Establishing what had happened then cost a full week's audit pull filtered client-side on object IDs, because Microsoft Entra records "Remove member from group" against the group rather than the user, so the portal's target-name filter on `John Smith` returned nothing at all. Separately, the "6 of 25" figure in that same Final State was recorded before anyone understood what the page was counting; it was never a wrong reading, it was an unexamined unit, and it took until Part C two steps later to establish the basis and append a dated correction rather than write one into a narrative that had been accurate at the time. Both cost the same thing. A number or a state recorded without being interrogated becomes something the document asserts, and later steps build on it. The habit the lab arrived at, and should have opened with, is that a figure read off a screen is not a finding until its unit is known, and that a Final State is a claim about one moment rather than a standing description of the tenant.
