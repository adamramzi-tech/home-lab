...
All twelve steps are complete: implemented, run against the live tenant and the on-premises environment, and documented in past tense below. The tenant holds Microsoft Entra ID Premium P1 through a Microsoft 365 Business Premium thirty-day trial with recurring billing off, expiring 2026-10-05, and finished at 10 users and 9 groups against a Step One baseline of 10 and 5. Business Premium stands at seven assignment targets against six consumed seats out of 25 enabled, the one-seat gap being a licensed group that holds no members. The only on-premises changes that persist are `department` populated on the six synchronized users and `title` on `mjohnson`.
...
**Decision:** The trial is started in Step Two of the Implementation section below, performed and screenshotted in sequence like any other configuration action, and the resulting licensing state is read back from the tenant in Step Four.
...
Steps Two through Ten all depend on the trial in some way, and the dependency is not uniform. Steps Six, Seven, and Eight need Microsoft Entra ID P1 specifically, for dynamic membership groups, role-assignable groups, and group-based licensing, and cannot be attempted at all without it. Steps Five, Nine, and Ten need something weaker but still real: live licenses on live accounts. Step Five is where those licenses get assigned at all, and there is nothing to assign without the trial; Steps Nine and Ten document whether a restored user comes back holding the licenses it had, and what happens to a group that carries a group-based license assignment when it is deleted. None of the three is answerable against accounts that hold nothing.
...
The baseline matched what was expected in every dimension the plan named, and it held to the end of the lab. Step Nine created a throwaway synchronized account, `deltest01`, and then destroyed it permanently, which in a forest with no Active Directory Recycle Bin was irreversible on-premises. Because that account was this lab's own creation rather than part of this baseline, its removal returned the user count to this number rather than moving it: Step Nine's own Final State and Step Twelve's reconciliation each confirmed the count unchanged on both sides. What the deletion cost was not a count but two distinct Entra object GUIDs along the way, which Step Nine records in full.

### Step Two: Started the Microsoft 365 Business Premium trial

From the Microsoft 365 admin center, Billing, Add more products, a Microsoft 365 Business Premium trial was started in the existing tenant. The tenant accepted it without objection, which settles the question this step existed to answer: a tenant that has already consumed a Microsoft 365 Business Basic trial is not blocked from starting a Microsoft 365 Business Premium trial in the same organization. No refusal was encountered, so the refusal path this step was written to handle never fired, and the lab did not have to stop for the decision Design Decisions reserved for that outcome.
...
Business Premium was assigned to five accounts: two cloud-only, `admin@brindeck.com` and `cloudonly-demo01`, and three synchronized, `testuser01`, Alex Kim, and Jane Doe, selected to put both object types on both sides of every licensing observation this lab makes rather than licensing one kind of account and inferring the other. The Emergency Access Account and the signup account were left untouched.
...
<p align="center">
  <em>IT-Admins' Properties page: Group name, Group description, and Membership type all greyed out with placeholder text only, under the "Some groups can't be managed in the Azure portal" banner. Finance's equivalent page, by contrast, showed all three fields live and editable.</em>
</p>
...
Before creating anything, the role-assignable constraint was tested against groups that already exist rather than taken from Microsoft's statement of it. From the Global Administrator account, Groups Administrator's Add assignments panel, filtered to its Groups tab, returned "No results found," despite the tenant holding eight groups by this point: four synchronized from `OU=Groups`, plus four cloud-only groups, `All Company`, `Finance` and `Company Announcements` from Step Six, and `IT-Department`, the dynamic membership group Step Six built. `IT-Department` is the strongest case in that set, not an afterthought: it is already excluded on a second, independent ground, since Microsoft's own role-assignable group documentation requires Assigned membership and `IT-Department` is Dynamic. Its absence from the results confirms both constraints hold at once, not only the one this test targeted. That rules out the plan's framing being only about synchronized groups: none of the tenant's eight existing groups qualified, cloud-only, dynamic, or neither, because none of them were created with role-assignability enabled, and the product does not offer a way to turn it on afterward.
...
The membership-type constraint Microsoft's role-assignable group documentation states, that such a group must use Assigned membership rather than Dynamic, was checked directly on the created group rather than left as an unexercised rule: `Groups-Administrators`' Properties page showed Membership type greyed out, not merely defaulted to Assigned but disabled from being changed at all once the group holds the role-assignable flag. The product enforces the constraint by locking the field, rather than by accepting a change and failing it afterward.
...
Deleted and restored a cloud-only user and a synchronized user, and documented how the two differ. The subjects were named deliberately, because the on-premises half of this step was not reversible in this forest: `corp.home.arpa` has no Active Directory Recycle Bin, Lab 02 having deferred that recommendation to the Enterprise Infrastructure track rather than enabling a forest-wide, irreversible setting mid-lab.
...
The two cases below are presented by object type, cloud-only first and synchronized second, rather than in the order they were run: Nine-C's work ran earlier on the same day, Sep 8, 2026, and its timestamps are correspondingly earlier than this sub-step's. Object type is the organizing axis of the whole comparison, and taking the fully reversible case before the terminal one is what makes the divergence between them legible; Nine-A above is the prerequisite check both depend on and did genuinely run first.

`cloudonly-demo01` was set up to exercise all three properties Design Decisions asked a restore to demonstrate, not license alone: it was added to `All Company`, the tenant's unlicensed, cloud-only, assigned-membership Microsoft 365 group, and assigned the `License Administrator` directory role, the same role Step Seven already validated safely against `testuser01`. `Finance` and `Company Announcements` were both ruled out as the group for this test: each carries a group-based license assignment from Step Eight, and adding either one would layer an Eight-C-style direct-versus-group collision on top of this read and confound it. `IT-Department` was ruled out too, its dynamic membership can't be edited by hand. The pre-delete baseline, read live rather than assumed: object Id `0c305ee2-...`, one direct license (`cbdc14ab-d96c-4c30-b9f4-6ada7cdc1d46`, Business Premium, unchanged since `All Company` carries none), and a `memberOf` of exactly two entries, `All Company` (`#microsoft.graph.group`) and `License Administrator` (`#microsoft.graph.directoryRole`).
...
Deleted in the tenant only, through the portal, at 12:28 PM on Sep 8, 2026, leaving the on-premises object untouched. Every clock time in this sub-step is that same morning. `Get-ADSyncScheduler` gave the next scheduled Delta cycle as 12:46:09 PM; rather than force it, the step waited for that scheduled run, since the point was to watch the same unattended half-hourly cycle Lab 02 Step Nine had to make reliable before it could run without anyone watching it. By 12:55 PM, `deltest01` resolved again through `Get-MgUser` with the identical object Id, `4dc7ca85-...`, confirming the scheduled cycle had restored it on its own. Nothing was checked between the 12:46:09 PM cycle and the 12:55 PM confirmation, so the honest window is eighteen to twenty-seven minutes after the delete, not a single precise figure, but the outcome is unambiguous either way: Design Decisions' prediction held. Microsoft Entra is not the source of authority for a synchronized object, so an Entra-side delete against a still-existing on-premises source does not stick.
...
This is the most instructive artifact this lab captured, and what earns it that is the message text rather than the refusal: it names the constraint by category, directory sync objects, rather than returning a bare permissions error that would have left the actual reason to guess at. The identical write against `cloudonly-demo01` succeeded silently and read back as set:
...
The historical `mobile` and `otherMobile` exception, which Microsoft's own `BypassDirSyncOverridesEnabled` documentation records as no longer applying and which this step therefore checked rather than assumed in either direction, did not hold on this tenant. The identical write against `mjohnson`,
...
<p align="center">
  <em>The Select groups picker for mjohnson: All Company and Company Announcements selectable, Finance and Groups-Administrators selectable, while Domain-Users-Standard, IT-Admins, Lab-Workstations, and Linux-Admins each read "Directory synced objects are not allowed." and IT-Department reads "Dynamic groups are not allowed.", every one of those five greyed and unselectable for its own stated reason.</em>
</p>

Both results ran against the same user, confirming the constraint belongs to the group's source of authority rather than the user's: the identical `mjohnson` could join a cloud-only group and could not even attempt to join a synchronized one. Once the "successfully added" evidence above was captured, `mjohnson` was removed from `All Company` again; nothing in this step's content depends on the membership persisting, and leaving it in place would have carried a test artifact into Step Twelve's reconciliation for no reason.
...
### Step Twelve: Validated the environment is otherwise unchanged, settled the SSO key, and recorded the finished state

Confirmed that a lab conducted almost entirely in two web portals left the on-premises environment exactly as it found it, and recorded the state Lab 04 starts from.

**Drift check, run before anything else.** `mjohnson`'s Overview, Job Information, and Groups tabs matched Step Eleven's Restoration paragraph exactly: UPN back at `mjohnson@brindeck.com`, `Account enabled: Yes`, Job title `IT Support Specialist`, and membership in exactly `Domain-Users-Standard` and `IT-Admins`.

<p align="center">
  <img src="../../images/cloud-and-hybrid-identity/03-entra-id-user-group-and-license-administration/44-mjohnson-groups-membership-restoration-confirmed.jpg" alt="44-mjohnson-groups-membership-restoration-confirmed" width="700">
</p>

<p align="center">
  <em>mjohnson's Groups blade: exactly Domain-Users-Standard and IT-Admins, both group object IDs masked.</em>
</p>

`cloudonly-demo01` matched too: zero group memberships, zero assigned roles, one assigned license (SPB), Job title cleared back to blank.

<p align="center">
  <img src="../../images/cloud-and-hybrid-identity/03-entra-id-user-group-and-license-administration/45-cloudonly-demo01-overview-license-only-restoration-confirmed.jpg" alt="45-cloudonly-demo01-overview-license-only-restoration-confirmed" width="700">
</p>

<p align="center">
  <em>cloudonly-demo01's Overview: Group memberships 0, Applications 0, Assigned roles 0, Assigned licenses 1, object ID masked.</em>
</p>

A fresh `Get-MgSubscribedSku` read reconfirmed Part C's figures unchanged: SPB (Business Premium) at 6 consumed units and Microsoft 365 Business Basic (no Teams) at 1, both `Enabled`, and the Business Premium product page still read 7 of 25 assigned. No drift on either front, so this step proceeded from Step Eleven's and Step Ten's own baselines rather than a re-derived one.

**On-premises and synchronization.** `Test-ComputerSecureChannel` from WIN11-CLIENT01 returned `True`. A first pass at the Group Policy result, `gpresult /r /scope:computer`, showed only `Workstation-Security-Baseline` and `Default Domain Policy`, no `IT-Admin-Environment` and briefly read as drift. It was an instrument gotcha instead: `IT-Admin-Environment` is a User Configuration policy security-filtered to `IT-Admins` membership, so a computer-scoped result can never show it regardless of whether it applies. A full `gpresult /r` for `CORP\labadmin`, a member of `IT-Admins` sitting in `OU=IT`, confirmed it correctly under User Settings.

`sssd` was `active (running)` on Ubuntu Server, but the cached `klist` ticket was seven days expired, `testuser01@CORP.HOME.ARPA` valid `09/03/2026 13:15:55` to `09/03/2026 23:15:55`, left over from the day the service last started rather than a currently valid ticket. A fresh `kinit testuser01@CORP.HOME.ARPA` produced a ticket correctly bracketing the current time. Both this stale ticket and a nine-second `sssd_be` offline/online pair logged at the moment of testing traced to the same cause: the VM had been suspended between lab sessions and had only just been resumed for this step, not a genuine connectivity fault.

`SYNC01` had been suspended the same way, and it surfaced this step's most substantial finding. `Get-ADSyncScheduler` read `SyncCycleEnabled: True` and `SchedulerSuspended: False`, but `NextSyncCycleStartTimeInUTC` at `9/9/2026 3:31:00 PM`, two days stale, and the Synchronization Service Manager's Operations tab confirmed no cycle had actually run since then either, 1,374 runs total, all `success`. The `ADSync` service itself was `Running`, and the Windows Application event log showed it actively acquiring MSAL tokens against `AdminWebService` moments before the check, alive but evidently not re-arming its own scheduler after the suspend. No Task Scheduler entry matched `ADSync` or `Azure AD Sync` (only an unrelated `TouchpadSyncDataAvailable` task matched the name filter), confirming the scheduler runs as an in-process timer inside the `ADSync` service rather than as an external scheduled task. `Restart-Service ADSync` did not correct it: `Get-ADSyncScheduler` still read `9/9/2026 3:31:00 PM` a full three minutes later. A full VM restart did: the next read gave `NextSyncCycleStartTimeInUTC: 9/11/2026 11:23:27 PM`, roughly the correct half hour out, nothing suspended. No cycle was ever forced manually at any point in this sequence, consistent with the discipline Lab 02 Step Nine established around this exact cmdlet. Nothing was actually missed by the roughly two-day gap: no lab work ran against either directory while `SYNC01` sat suspended, so there was no on-premises change waiting to synchronize when the scheduler finally caught up. This is recorded as a carry-forward item for this track's close-out to propagate into the `README`: any future session that resumes a suspended `SYNC01` should expect the identical stale scheduler, and know in advance that a service restart will not correct it where a full VM restart will.

<p align="center">
  <img src="../../images/cloud-and-hybrid-identity/03-entra-id-user-group-and-license-administration/46-adsyncscheduler-next-cycle-corrected-after-vm-restart.jpg" alt="46-adsyncscheduler-next-cycle-corrected-after-vm-restart" width="700">
</p>

<p align="center">
  <em>Get-ADSyncScheduler after the SYNC01 VM restart: NextSyncCycleStartTimeInUTC recalculated to 9/11/2026 11:23:27 PM, SyncCycleInProgress False, nothing suspended.</em>
</p>

The Entra Connect version and source anchor were both unchanged: `Microsoft Entra Connect Sync 2.6.84.0` (found under that exact product name in the uninstall list, once `Microsoft Azure AD Connect Agent Updater`, a different component with its own version number, was ruled out), and the wizard's "View current configuration" page still read `mS-DS-ConsistencyGuid` as source anchor, the attribute this track chose specifically because it survives an on-premises object being deleted and recreated, unlike `objectGUID`, exactly the property Step Nine's `deltest01` finding already leaned on.

**Wazuh, the gap confirmed rather than repeated.** `Invoke-LabHealthReport.ps1`'s `WazuhAgentStatus` check queried only `DC01`, `WIN11-CLIENT01`, and `UBUNTU-SERVER`, its hardcoded default from Automation Lab 05, all `active`, and returned `Overall: Healthy` without ever having asked about `SYNC01`. The explicit call, `Get-LabWazuhAgentStatus -AgentName DC01,WIN11-CLIENT01,UBUNTU-SERVER,SYNC01`, showed all four `active`/`Healthy`, `SYNC01` included. The two results differ because the default script structurally excludes the one host this track's synchronization depends on, not because anything was actually wrong with `SYNC01` this time; a green `Overall: Healthy` from the unmodified script would have been correct by coincidence, not because the check covered what mattered.

<p align="center">
  <img src="../../images/cloud-and-hybrid-identity/03-entra-id-user-group-and-license-administration/47-get-labwazuhagentstatus-explicit-four-agents-including-sync01.jpg" alt="47-get-labwazuhagentstatus-explicit-four-agents-including-sync01" width="700">
</p>

<p align="center">
  <em>Get-LabWazuhAgentStatus called explicitly with SYNC01 added to the default agent list: all four hosts active and Healthy.</em>
</p>

**Automation library.** `Invoke-Pester -Path C:\Scripts -Output Detailed` completed in 39.37 seconds: 174 tests passed, 0 failed, 0 skipped, 0 not run, unchanged, since no script is touched by this lab.

<p align="center">
  <img src="../../images/cloud-and-hybrid-identity/03-entra-id-user-group-and-license-administration/48-invoke-pester-174-passed-0-failed.jpg" alt="48-invoke-pester-174-passed-0-failed" width="700">
</p>

<p align="center">
  <em>Invoke-Pester summary: 174 tests passed, 0 failed, 0 skipped, 0 not run.</em>
</p>

**The `AZUREADSSOACC` key.** A fresh `Get-ADComputer AZUREADSSOACC -Properties PasswordLastSet` read on `SYNC01` returned `8/31/2026 8:07:44 PM`, unchanged from Step One: eleven days elapsed as of this step, well short of the thirty the interval names.

<p align="center">
  <img src="../../images/cloud-and-hybrid-identity/03-entra-id-user-group-and-license-administration/49-azureadssoacc-passwordlastset-fresh-read.jpg" alt="49-azureadssoacc-passwordlastset-fresh-read" width="700">
</p>

<p align="center">
  <em>Get-ADComputer AZUREADSSOACC -Properties PasswordLastSet, re-read on SYNC01 so the elapsed interval spans this step: 8/31/2026 8:07:44 PM, unchanged from Step One.</em>
</p>

Microsoft Learn's current wording, checked directly rather than assumed from the plan, since Lab 02 already found this documentation set does not always match its own cmdlets, is consistent across its Quickstart, Technical Deep Dive, and FAQ articles: "we highly recommend" rolling the key "at least every 30 days." None of them describe the key as expiring, and Defender for Identity's own automated security assessment for this account does not flag it until the password is over ninety days old, not thirty. That settles the question this lab was built to answer: thirty days is a hygiene interval, not an operational deadline the key stops functioning at. The decision taken was not to roll the key in this step. Eleven days is well short of thirty, rolling now would only repeat the demonstration Lab 02's Step Eight-A already gave, and it would erase the one chance this track has to watch a full thirty-day interval actually elapse on 2026-09-30 (2026-10-01 in UTC), which a later lab is meant to observe empirically rather than take on faith.

**Finished state.** Reconciled both directories against the Step One baseline. The Entra Overview read 10 users, unchanged, and 9 groups, which is unchanged from Step Ten's own close rather than untouched by this lab overall: against Step One's baseline of 5 (the four groups synchronized from `OU=Groups` plus `All Company`), Step Six added `Finance`, `Company Announcements`, and the dynamic membership group `IT-Department`, and Step Seven added the role-assignable group `Groups-Administrators`, all four of which persist unchanged into this step. On-premises, `(Get-ADUser -Filter *).Count` returned 12, not a discrepancy once decomposed: `akim`, `jdoe`, `jsmith`, `mjohnson`, `tsync01`, and `testuser01` in `OU=User Accounts` are the six synchronized users the baseline has always tracked; the other six, `Administrator`, `Guest`, and `krbtgt` (built-in), `labadmin` (the `IT-Admins` admin account in `OU=IT`), `svc-entraconnect` (the Entra Connect service account in its own `OU=Service Accounts`), and `Test NoSync` (`tnosync01`, deliberately placed in `OU=IT` as this track's own fixture proving synchronization's OU scoping), were never part of that six-user count. `deltest01`, created and permanently destroyed in Step Nine, nets to zero on both sides exactly as Step Nine already confirmed, at the cost of two distinct Entra object GUIDs along the way rather than a persistent count, so it is accounted for here rather than netted silently away. The two attribute changes this lab makes on-premises, `department` from Step Six and `mjohnson`'s `title` from Step Eleven, both persist as deliberate content per the precedent those steps already set.

Licensing, read fresh rather than carried forward: Business Premium (SPB) remained at 7 assignment targets against `Get-MgSubscribedSku`'s 6 consumed seats, 25 enabled, `Finance`'s empty membership accounting for the one-target gap exactly as Part C established. Business Basic (no Teams) showed the identical pattern on inspection, and this is not a re-read of Part C but a second, independent instance of its finding on a different SKU: the product page read 2 of 25 assigned against `Get-MgSubscribedSku`'s 1 consumed; the export identified the two assignment targets as `Finance` (the group, still empty) and `Adam Ramzi` (a user), so `Finance` again contributes zero of its own consumed seats and the arithmetic closes the same way Part C's did for SPB. It carries the same unexplained movement with it, though, and that half is not a confirmation of anything. `Finance` became a Business Basic assignment target in Eight-E, so under the target-counting basis Part C establishes the page should have read 2 of 25 from that moment; Step Eight's Final State records it at 1 of 25 instead, and nothing between that reading and this one added a target. That is the same shape as the Business Premium page moving from 6 to 7 with only a group membership changing under it, recorded as unresolved in Part C, and it is recorded the same way here. No mechanism is proposed for either.

<p align="center">
  <img src="../../images/cloud-and-hybrid-identity/03-entra-id-user-group-and-license-administration/50-licensing-reconciliation-consumedunits-and-business-basic-targets.jpg" alt="50-licensing-reconciliation-consumedunits-and-business-basic-targets" width="700">
</p>

<p align="center">
  <em>Get-MgSubscribedSku narrowed to SkuPartNumber, ConsumedUnits, and CapabilityStatus, paired with the Business Basic (no Teams) product page: 2 of 25 assigned against 1 consumed, Finance and Adam Ramzi the two assignment targets.</em>
</p>

The Business Basic trial's expiration read `9/22/2026`, resolving the track's dated item at last. Microsoft Entra ID P1 is carried entirely by the Business Premium trial rather than held independently of it, so it lapses with that trial on 2026-10-05 and takes dynamic membership evaluation, role-assignability, and group-based licensing with it. Microsoft Entra ID Free remained on the Your products page as its own non-assignable line throughout, exactly as Steps One and Three recorded it, and is what the tenant falls back to.

The reconciliation also accounted for the four objects invisible to the Entra Overview's counts, each decided rather than defaulted past. `nolocation-demo01` (soft-deleted 9/7/2026 4:01 PM, permanent deletion 10/7/2026 4:01 PM) was left for Labs 04 and 05 to inherit: it is the concrete object behind Part C's Finding Two, a soft-deleted user that keeps its `assignedLicenses` property while consuming no seat, and purging it now would destroy the artifact this lab worked to establish for no benefit. `duptest01` and `duptest02` (both soft-deleted 9/2/2026 9:41 PM, permanent deletion 10/2/2026 9:41 PM), which Step Ten already confirmed hold no licenses, were purged permanently: Lab 02's own record claims it "left no residue," which was false while these two persisted, and purging them makes that claim true rather than handing the same stale discrepancy to a later lab to rediscover. `Testgroup` (soft-deleted 9/6/2026 5:08:17 PM, permanent deletion 10/6/2026 5:08:17 PM), which predates this lab entirely and was never touched or explained by any step in it, was left for Labs 04 and 05 to inherit: this session had no basis to restore it, since its original composition and purpose are unknown, and no basis to purge it either, since that would destroy an artifact of unknown provenance; leaving it preserves the option for whichever later lab has the context to decide.

<p align="center">
  <img src="../../images/cloud-and-hybrid-identity/03-entra-id-user-group-and-license-administration/51-deleted-users-after-permanent-purge-nolocation-demo01-remains.jpg" alt="51-deleted-users-after-permanent-purge-nolocation-demo01-remains" width="700">
</p>

<p align="center">
  <em>Deleted users after the permanent purge of duptest01 and duptest02: only nolocation-demo01 remains, both identifying values masked.</em>
</p>

Deleted groups still held exactly `Testgroup`, unchanged and untouched, closing the reconciliation.

---

## Validation

Every item below was checked against the tenant or the on-premises environment at the point the lab reached it. Two planned items could not be performed as written, and both are recorded as they happened rather than reworded until they pass.

- **Licensing.** Met. A Microsoft 365 Business Premium trial was active in the tenant from Step Two, with 25 licenses confirmed on the subscription's own page at 0 of 25 assigned. Its end date was recorded as the portal reported it rather than reconciled into one figure: the subscription page's Expiration date and the Your products column both read 10/5/2026, while the same page's Recurring billing field read "Expires on October 6, 2026." Recurring billing was turned off on the Business Premium trial in Step Three, in the same sitting it started, and the Business Basic trial was reconfirmed lapsing rather than converting, its 9/22/2026 expiration re-read fresh at Step Twelve. The Entra admin center Overview reported the tenant's Microsoft Entra plan as Entra ID Premium P1, with License usage recording Entra ID P1 at 25 and Licensed features showing Advanced Group Access Management as available while the four Microsoft Entra ID P2 features listed beside it correctly were not. Microsoft Entra ID P1, Exchange Online Plan 1, and Microsoft Intune Plan 1 were all confirmed present by service plan name, but not where the plan expected to find them. Neither the Your products subscription page nor the dedicated Billing, Licenses page exposes a service plan breakdown before assignment, and no Apps or service-plan tab is reachable from either; `AAD_PREMIUM` was confirmable independently from the Entra admin center, and `EXCHANGE_S_STANDARD` and `INTUNE_A` had no Entra-side equivalent at all. All three were read directly off the SKU with `Get-MgSubscribedSku` from WIN11-CLIENT01 and cross-referenced against Microsoft's service plan identifier reference rather than inferred from the internal names. Trial eligibility was recorded as a finding rather than assumed: the tenant accepted a Business Premium trial without objection despite having already consumed a Business Basic trial, so the refusal path Step Two was written to handle never fired.

- **Scope guard.** Met. Step Four confirmed Conditional Access, Policies showing the empty getting-started view a tenant with no policies displays, and security defaults' own settings panel reading "Enabled," with "Your organization is currently using security defaults" in the portal's own words. No step from Five through Twelve created, modified, or evaluated a conditional access policy, and none touched security defaults. The close-of-lab state is that continuity rather than a second independent reading taken at Step Twelve.

- **License assignment models.** Partially met. Both assignment models were demonstrated; the deliberate error was not produced. Direct per-user assignment ran against five accounts in Step Five, two cloud-only and three synchronized, with no behavioral difference between the two object types at any point. Group-based licensing was demonstrated on both sides of the grant in Step Eight: `John Smith`, holding no license of any kind, drew Business Premium through `Finance` membership alone with no visible delay, and lost it entirely the moment that membership ended. He was then moved between two licensed groups in Microsoft's documented order, added to `Company Announcements` and confirmed before being removed from `Finance`, and his license stayed checked throughout with no interruption. No license assignment error was induced. Three routes were attempted and none produced one: an account created with the "Create user without product license" option and added straight to a licensed group still resolved to a United States usage location and licensed cleanly; two SKUs supplied to the same account through the same group both applied without conflict; and insufficient licenses was never attempted, because a 25-seat trial against a ten-user tenant cannot reach it. The Errors and issues tab was read directly and held zero licensing errors and zero members without licenses. What the step produced instead was an unplanned failure on a path it was not testing: deleting that account returned "We couldn't unassign licenses for this user. One or more of the licenses could not be modified because they are inherited from a group membership," a partial failure that did not block the deletion and that Step Nine then traced to a soft-deleted object still carrying both SKUs.

- **Groups.** Met, with one sub-question closed as unanswerable rather than answered. All nine groups the tenant holds at the close are accounted for by type, membership type, source, and administration point. `All Company`'s membership governance was settled directly: a cloud-sourced Microsoft 365 group with Assigned membership, whose description claims it is the default group for everyone in the network while nothing about the object enforces that, and whose only member is the cloud-only Global Administrator retained since Lab 01. Its origin was not settled. Audit log retention in this tenant reaches back only to 2026-08-31, eight days short of the group's 2026-08-23 creation, so the creation event had already rolled off and no evidence survives to distinguish tenant provisioning from a manual action. `Finance` and `Company Announcements` were created as the environment's first cloud-only security group and Microsoft 365 group, and their Properties and Members pages were compared field by field against `IT-Admins`, whose equivalents were greyed to placeholder text behind the "Some groups can't be managed in the Azure portal" banner. `IT-Admins`' partial membership was confirmed with its reason: three members in the tenant against four on-premises, because `labadmin` sits in the deliberately unsynchronized `OU=IT`, and nothing in the portal indicates the list is incomplete.

- **Dynamic membership.** Met in full. `department` was populated on all six synchronized users from WIN11-CLIENT01 with `Set-ADUser`, starting from confirmed-blank across the board. That it arrives in the tenant without a custom synchronization rule was confirmed two ways rather than assumed: it appears in Microsoft's synchronized attributes reference outside the bare-minimum set, and `Get-MgUser` read the values back after a Delta cycle matching the on-premises values exactly. It was confirmed referenceable in a rule directly, appearing as a selectable item in the rule builder's Property picker rather than requiring the raw syntax box. `IT-Department` exists as a Security group with Dynamic User membership on the rule `(user.department -eq "IT")`, its logic confirmed live by Validate Rules against five of the six synchronized users before the group's own membership had finished materializing. The round trip was timed end to end rather than described: `Set-ADUser -Identity jsmith -Department "IT"` landed at 3:40:25 PM, and the group's Last membership change read 3:45 PM, which the portal's minute-level precision makes a range of 4 minutes 35 seconds to 5 minutes 34 seconds rather than a single figure. The write permissions check ran in Active Directory with `dsacls` and returned the opposite of what Microsoft's general warning predicts: `NT AUTHORITY\SELF` holds `WRITE PROPERTY` on four property sets, `department` belongs to none of them, and the only ACE naming its Public-Information property set grants `READ PROPERTY` to `Authenticated Users` with no write grant at all, so a synchronized user on this schema cannot add themselves to the group by editing their own attribute. `New-LabUser.ps1`'s failure to set organizational attributes was recorded as a Lab 06 candidate and left unfixed.

- **Directory roles.** Met, and the synchronized-group half of the bullet proved narrower than the tenant's actual constraint. License Administrator was assigned to `testuser01` at Directory scope, Assignment type Active rather than Eligible because Privileged Identity Management requires Microsoft Entra ID P2 and this tenant holds P1. Its effective permissions were confirmed by signing in as that account and removing and then restoring another user's license from the Microsoft 365 admin center, which is where Microsoft has put the control; the Entra admin center's own Licenses blade is read-only for every role and says so in a banner. `Groups-Administrators` exists as the tenant's first role-assignable group and holds Groups Administrator at Directory scope, confirmed from the role's Assignments tab and from the group's own Assigned roles blade. The claim about the four synchronized groups was confirmed and exceeded: Groups Administrator's Add assignments panel returned "No results found" against all eight groups the tenant then held, the four cloud-only ones included, because role-assignability is set at creation and the product offers no way to enable it afterward. One negative result was corrected rather than recorded as observed. `testuser01` successfully created a group, which read as a role capability and was not one: Identity, Groups, General showed self-service security group and Microsoft 365 group creation both set to Yes at the tenant level for any authenticated member, untouched since Lab 01.

- **Synchronized versus cloud-only.** Met in substance, not in the form the bullet names. The comparison was recorded as prose across Eleven-A through Eleven-E, and the documented table of attempted actions this item calls for does not exist in the document. What the prose does record, per action, is what the table was meant to hold. Job title and Mobile were both refused on the synchronized user, `Update-MgUser` returning Status 400, ErrorCode `Request_BadRequest`, and a message naming on-premises mastered Directory Sync objects by category rather than a bare permissions error. The identical Job title write succeeded silently against `cloudonly-demo01` and read back as set. Group membership succeeded into a cloud-only group and was refused in the Select groups picker itself for every synchronized group, greyed with "Directory synced objects are not allowed." before any save could be attempted, with `IT-Department` greyed separately for being dynamic. The refused change was then made on-premises and watched arriving: `Set-ADUser -Identity mjohnson -Title "IT Support Specialist"` at 1:17:00 AM and the export to the tenant completing at 1:22:01 AM, a round trip of 5 minutes 1 second read from the Synchronization Service Manager's log rather than the portal's minute-level clock. Two planned expectations did not hold. The historical `mobile` and `otherMobile` override did not apply, the write returning the identical refusal, consistent with the tenant's own `BypassDirSyncOverridesEnabled` reading `False`. And the lock proved to be per field rather than per object, with user principal name and account enabled state both writable in the cloud on a synchronized object; the first was explained afterward by `SynchronizeUpnForManagedUsersEnabled` reading `True` against a managed, unlicensed user rather than left as a bare surprise, and the durability of the second was recorded as an open question rather than claimed from a disable-and-re-enable that no inbound change ran between.

- **Delete and restore.** Met in full, and the documentation contradiction is settled. Both user object types were deleted and restored using `cloudonly-demo01` and the throwaway `deltest01`, with no account Lab 04 or Lab 05 depends on touched. The cloud-only restore returned all three properties rather than licensing alone: the same license, the same `All Company` membership, and the same License Administrator assignment, each confirmed by re-running the pre-delete Graph queries against the restored object. The synchronized user deleted only in the cloud was observed across a scheduled cycle rather than a forced one, which is the whole point of the test: deleted at 12:28 PM against a next scheduled Delta of 12:46:09 PM, it resolved again through `Get-MgUser` by 12:55 PM under the identical object Id, an honest window of eighteen to twenty-seven minutes. The terminal case followed and did not undo itself. `Remove-ADUser` at 12:57 PM, in a forest with no Active Directory Recycle Bin, left no Deleted Objects container behind it; the 1:17:16 PM cycle carried the deletion to the tenant, and the same object was soft-deleted a second time at 1:20 PM with nothing on-premises left to restore it from. Recreating the account on-premises with the identical `sAMAccountName` and UPN provisioned an unrelated second cloud object, because a new `objectGUID` means a new `ms-DS-ConsistencyGuid` source anchor, leaving the original orphaned in Deleted users with its thirty-day window still technically open and no source left to claim it. The security group question was settled against the tenant rather than against either document: a cloud-only security group appeared on Deleted groups beside a cloud-only Microsoft 365 group, both carrying 10/8/2026 permanent deletion dates, and both restored with membership intact. Microsoft's recoverability architecture guidance held; the Entra PowerShell reference's claim that security groups cannot be restored did not.

- **Seamless SSO key.** Met. `Get-ADComputer AZUREADSSOACC -Properties PasswordLastSet` returned 8/31/2026 8:07:44 PM at Step One and the identical value re-read at Step Twelve, eleven days elapsed rather than thirty. The recommendation was characterized on evidence rather than preference: Microsoft's Quickstart, Technical Deep Dive, and FAQ agree on "we highly recommend" rolling the key "at least every 30 days," none of them describes the key as expiring, the Quickstart states explicitly that the roll is not needed immediately after enabling the feature, and Defender for Identity's own hybrid posture assessment does not flag the account until its password is over ninety days old. Thirty days is a hygiene interval, not an operational deadline. The key was deliberately not rolled, with the reasoning recorded: eleven days is well short of thirty, rolling now would only repeat the demonstration Lab 02 Step Eight-A already gave, and it would erase this track's one chance to watch a full interval actually elapse on 2026-09-30.

- **Environment unchanged.** Met for every host and service except the synchronization scheduler, which was found stale and repaired in Step Twelve rather than carried forward. `Test-ComputerSecureChannel` from WIN11-CLIENT01 returned `True`. A first `gpresult /r /scope:computer` missing `IT-Admin-Environment` read briefly as drift and was an instrument gotcha instead: a User Configuration policy security-filtered to `IT-Admins` membership cannot appear in a computer-scoped result regardless of whether it applies, and a full `gpresult /r` for `CORP\labadmin` confirmed it correctly under User Settings. On Ubuntu Server, `sssd` was `active (running)`, and both a seven-day-expired cached `klist` ticket and a nine-second `sssd_be` offline and online pair traced to the VM having been suspended between sittings rather than to any connectivity fault; a fresh `kinit` produced a ticket correctly bracketing the current time. `SYNC01` had been suspended the same way and there the consequence was real: its synchronization scheduler had not fired in two days while every status property it exposes still reported healthy. It was diagnosed and repaired within this step rather than carried forward, by a full VM restart after a service restart proved insufficient; Troubleshooting and Adjustments carries the diagnosis. Nothing was missed by the gap, since no directory change was pending while the VM sat suspended. Entra Connect Sync 2.6.84.0 and `mS-DS-ConsistencyGuid` as source anchor were both unchanged. All four Wazuh agents were confirmed `active` by an explicit agent list, while `Invoke-LabHealthReport.ps1` returned `Overall: Healthy` from its hardcoded three-host default without ever asking about `SYNC01`, the blind spot recorded again rather than repaired here. The Pester suite passed 174 tests, 0 failed, 0 skipped, unchanged, since this lab touched no script. The only on-premises directory changes are the three deliberate ones: `department` populated on six synchronized users in Step Six, `title` populated on `mjohnson` in Step Eleven, and `deltest01` created and destroyed in Step Nine.
...
- **Suspending the `SYNC01` VM leaves the Entra Connect scheduler's in-process timer stale, and a service restart does not fix it.** Step Twelve found `Get-ADSyncScheduler` reporting `SyncCycleEnabled: True` and `SchedulerSuspended: False` while `NextSyncCycleStartTimeInUTC` sat two days in the past; the Synchronization Service Manager's Operations tab confirmed no cycle had actually run in that window either. The `ADSync` service itself reported `Running` and was actively acquiring MSAL tokens against `AdminWebService`, so the service was alive, not crashed. No Task Scheduler entry exists for it under any name resembling `ADSync` or `Azure AD Sync`, which is what establishes that the scheduler runs as an in-process timer inside the service rather than as an externally triggered task, and that is the detail that explains why `Restart-Service ADSync` alone is insufficient: restarting the service does not reset the timer state a VM suspend corrupted, only a full VM restart does. Confirmed by testing both in sequence rather than assuming the fix from the symptom.
...
This lab was the first in the track to operate the hybrid environment rather than build it. Nothing was deployed and no topology changed. What changed is the tenant's entitlement, its object population, and the amount that is now known about how the two directories behave under ordinary administrative pressure.

The tenant moved from Microsoft Entra ID Free to Microsoft Entra ID Premium P1, carried by a Microsoft 365 Business Premium thirty-day trial started from inside the existing organization in Step Two with 25 licenses, recurring billing turned off in the same sitting so that it lapses rather than converts. The trial's service plans were confirmed by name off the SKU rather than inferred from the tier: `AAD_PREMIUM`, `EXCHANGE_S_STANDARD`, and `INTUNE_A` all provisioned, which is what makes this lab's P1 work, Lab 04's mailboxes, and Lab 05's device management a single subscription rather than three. Security defaults remains enabled and no conditional access policy exists, so the entitlement that makes conditional access available arrived without the obligation that comes with using it.

Licensing ends the lab in two shapes that do not agree by arithmetic and both of which are correct. Business Premium stands at seven assignment targets against six consumed seats, out of 25 enabled. Five of the targets are directly licensed users, Alex Kim, the Cloud Administrator, `cloudonly-demo01`, Jane Doe, and `testuser01`; two are groups, `Company Announcements` and `Finance`. The sixth consumed seat is `John Smith`'s, sourced entirely through `Company Announcements` membership and holding no target row of its own. `Finance` is licensed and empty, so it contributes a target and no seat, and that single empty group is the whole of the gap between the two figures. Business Basic (no Teams) shows the identical pattern on a smaller scale: two assignment targets, `Finance` and the signup account, against one consumed seat.

Nine groups persist against a Step One baseline of five. The four synchronized from `OU=Groups` are unchanged and unmanageable in the cloud. `All Company` is unchanged. The four this lab created all persist as deliberate content: `Finance`, a cloud-only assigned security group carrying both SKUs at the group level and no members; `Company Announcements`, a cloud-only assigned Microsoft 365 group carrying Business Premium with `John Smith` as its member; `IT-Department`, the environment's first dynamic membership group, on `(user.department -eq "IT")` with five members; and `Groups-Administrators`, the tenant's first role-assignable group, holding Groups Administrator at Directory scope with its Membership type field locked by the product. `testuser01` retains License Administrator at Directory scope. Both licensed groups are now known to be undeletable while those assignments stand, which is a constraint Lab 04 inherits rather than a state it can clean up in passing.

Four objects sit outside the Entra Overview's counts and each was decided rather than defaulted past. `nolocation-demo01`, soft-deleted 2026-09-07 with permanent deletion 2026-10-07, was kept: it is the concrete artifact behind this lab's finding that a soft-deleted object retains its `assignedLicenses` while consuming no seat, and purging it would destroy that for no benefit. `duptest01` and `duptest02`, soft-deleted since Lab 02 and confirmed to hold no licenses, were purged permanently, because Lab 02's record claims its cleanup left no residue and that claim was false while those two persisted. `Testgroup`, soft-deleted 2026-09-06 with permanent deletion 2026-10-06, predates this lab entirely and was left alone in both directions: there was no basis to restore an object of unknown composition and no basis to purge an artifact of unknown provenance, so the option passes to whichever later lab has the context to use it.

On-premises, the environment is as Lab 02 left it apart from two deliberate attribute changes and one account that nets to zero. `department` is now populated on all six synchronized users, at `IT` for `akim`, `jdoe`, `jsmith`, `testuser01`, and `tsync01` and at `Sales` for `mjohnson`, on a domain that carried no organizational attributes at all before this lab. `title` is populated on `mjohnson` alone, at `IT Support Specialist`. Both persist as content rather than being reverted. `deltest01` was created and destroyed on both sides, netting to zero at the cost of two distinct Entra object GUIDs along the way. Entra Connect Sync 2.6.84.0 and `mS-DS-ConsistencyGuid` are unchanged, the scheduler is running on its half-hourly Delta cycle again after the VM restart that a service restart could not accomplish, all four Wazuh agents are active, and the automation library is untouched at 174 passing Pester tests.

Lab 04 inherits a licensed population, an open set of questions, and three dates. On 2026-09-22 the Business Basic (no Teams) trial lapses, which costs the signup account its only license and, more usefully, makes observable what a group-level license assignment does when the subscription behind it goes away, since `Finance` carries one. On 2026-10-05 the Business Premium trial expires, and with it Microsoft Entra ID P1 and everything built on it: `IT-Department`'s rule evaluation, `Groups-Administrators`' role-assignability, and both groups' group-based licensing, none of which announce when they stop. The portal's own Recurring billing field puts that a day later at 2026-10-06, and the discrepancy is recorded rather than resolved. Separately, 2026-09-30 is thirty days since the `AZUREADSSOACC` key roll, an interval this lab deliberately left unrolled so that a later lab can watch it elapse against a recommendation now established as hygiene rather than a deadline. Five questions pass forward unresolved: why the directory surfaces, portal blade and Microsoft Graph alike, report nothing about a group-to-license relationship the commerce system enforces on deletion; why the Business Premium product page moved from six to seven with only a group membership changing under a counting basis that should not have moved, and why the Business Basic page moved from one to two with nothing changing under it at all; what unit each of the three service counts this lab read off the same Business Premium SKU is actually counting, since Step Four's Microsoft Graph read found 62 service plans, Step Five's Microsoft 365 admin center found 60 apps, and Step Seven's Entra admin center Licenses blade found 53 enabled services, and no two of the three were reconciled against each other; whether `New-LabUser.ps1` should set organizational attributes, which belongs to Lab 06 under the ADR-017 standard; and what a resumed `SYNC01` does to its own scheduler, which belongs in the track README for any session that suspends that VM.

---

## Lessons Learned

**A green status that reports configuration rather than work will report success while nothing is happening, and this environment has now produced that failure four times in three technologies.** Lab 02 found `Get-ADSyncScheduler` reporting `SyncCycleEnabled: True` when the scheduler had been disabled since installation and every change had reached the tenant by hand. [Linux Lab 06](../linux-infrastructure/06-monitoring-stack-lab.md) was revised for a Node Exporter that reported a successful Prometheus scrape while it had no access at all to the host's filesystem, disk, or kernel metrics. `Get-LabWazuhAgentStatus.ps1` has returned `Overall: Healthy` since `SYNC01` was built, from a default agent list that has never contained it. And Step Twelve found the `ADSync` service reporting `Running` and actively acquiring MSAL tokens while its in-process timer sat two days stale and no cycle had run at all. Four instances is a pattern rather than four coincidences, and naming it as one is worth more than four separate observations. The shared mechanism is that in every case the indicator is true about the thing it names and silent about the thing that matters, and the gap is only visible by reading a history rather than a state: the Operations tab, the scrape's actual metric set, the agent list the script was given. The practical form is a question to ask of any component reporting healthy, which is what the check would have had to do in order to notice the opposite. Step Twelve also produced the corollary. `Restart-Service ADSync` did not fix the stale timer, because restarting the service named in the status treats the status as the fault; only a full VM restart reset the state the suspend had corrupted.

**Two systems of record can measure different things, disagree by a whole number, and both be correct, and the way to settle that is to enumerate rather than to recompute.** The Business Premium product page read seven assigned and `Get-MgSubscribedSku` read six consumed, within minutes of each other against an identical tenant. Neither is wrong. The page counts assignment targets, the cmdlet counts seats a live user actually holds, and nothing on either surface says which unit it is using. The page's rolled-up view could not resolve it either, because a group-sourced holder appears under the group's row rather than his own, so the only thing that actually settled it was the page's own CSV export listing seven rows by name. Stated operationally, the page overstates real consumption by exactly one for every licensed group holding no members, which is a purchasing error waiting to be made by an administrator who reads one number and plans a renewal against it. The habit worth taking is that a disagreement between two authoritative readouts is a question about units before it is a question about correctness. One instance survived even that: the same composition read six on 9/7 and seven on 9/8 with only a group membership changing between them, which target-counting does not explain, and recording that as unresolved was better than folding it into a reconciliation that otherwise closes exactly.

**A platform can enforce a relationship on deletion while its own directory properties report that the relationship does not exist.** `Remove-MgGroup` refused with `400 Request_BadRequest` and the message "A group with active modern licenses assigned cannot be deleted," on a group whose `assignedLicenses` property returned an empty `{}` from a raw Microsoft Graph call, on a member whose `licenseAssignmentStates.assignedByGroup` read `null`, and after a repair and a fresh connection had ruled out both staleness and permissions. Two surfaces confirm the relationship exists, the Microsoft 365 admin center and the refusal itself, and two deny it, the Entra admin center's group Licenses blade and Graph. That is still unresolved and is recorded as a finding rather than explained. What it teaches in the meantime is that a refusal is evidence about state, sometimes the only evidence available, and that automation which decides what to do by reading a property will make confident wrong decisions where automation that attempts the operation and reads the error will not.

**Deletion is a spectrum rather than a binary, and this tenant produced five distinct positions on it in one lab.** A cloud-only user soft-deleted with every property retained, license, group membership, and directory role alike, and restored in a click. A soft-deleted user retaining its `assignedLicenses` property while consuming no seat, the property and the entitlement tracked separately so that only one of them survives the object's absence from the live population. A group that cannot be deleted at all while it carries an active license assignment. A synchronized user deleted in the cloud and restored eighteen to twenty-seven minutes later by a scheduled cycle nobody triggered. And an on-premises deletion in a forest with no Active Directory Recycle Bin, which is terminal, and whose recreated account arrives as an unrelated new cloud object under a new source anchor while the original sits orphaned in Deleted users with its window still open and nothing left that can claim it. "Someone deleted a user, can we get them back" has five different answers in this one tenant depending on where the object came from and what it was holding. The one that feels safest is the one that should worry an administrator most: a synchronized user reappearing on its own means the deletion did not do what it appeared to do, which is an inconsistency between the administrator's intent and the directory's belief rather than a safety net.

**When two first-party documents contradict each other, the cheap move is to pick the more authoritative-looking one and the correct move is to run the test.** Microsoft's Entra PowerShell reference states that only Microsoft 365 groups can be restored and that security groups cannot; its recoverability architecture guidance states that soft-deleted Microsoft 365 groups and cloud security groups both appear on Deleted groups and both restore. Both still say so. The tenant restored a cloud-only security group with its membership intact, which settles it for this environment permanently at the cost of one disposable group and four minutes. This is the second time in this track that Microsoft's documentation has lost to the tenant, after Lab 02 found `Get-EntraDirSyncFeature`'s accepted feature name did not match its own reference, which is often enough to treat a documentation conflict as a cue to test rather than a cue to read more carefully.

**Error messages frequently describe a symptom and name the wrong cause, and the diagnostic move is to drop a layer rather than investigate the category the message named.** The portal refused a group deletion twice with "Failed to delete group. Unable to complete due to service connection error. Please try again later," which described a transient network condition for a business rule that was never going to stop applying, and offered advice that could not have worked on any retry. Going straight to `Remove-MgGroup` to bypass the portal's wrapper produced the real error and the finding built on it. The same step produced the inverse trap: a `403 Authorization_RequestDenied` that read as a missing role on an account confirmed to hold Global Administrator with more than sufficient scopes already in session, and that turned out to be a stale token a `Disconnect-MgGraph` and reconnect cleared. So a generic message whose category the evidence contradicts calls for a layer down, and a permissions error on confirmed permissions calls for clearing the session before adding scopes. The counter-example in this lab is the one that shows what good looks like: `Update-MgUser`'s refusal naming on-premises mastered Directory Sync objects states the category of the constraint, which left nothing to guess at, and is why it is the most instructive artifact the lab captured.

**Instruments fail in ways that look exactly like data, and the tell is output that is uniform rather than merely empty.** Twice in this lab a query returned nothing and the nothing was the tool. `Get-MgDirectoryDeletedItem -Property '*'` returned a soft-deleted object with every extended property blank, `deletedDateTime` included, and `Get-MgGroup -Property AssignedLicenses` did the same against a live, populated group across a nine-minute wait, a variable check confirming real non-null objects, and a full reconnect with extra scopes; a raw `Invoke-MgGraphRequest` returned real data immediately in both cases, and the same raw call correctly returned a member for a group whose license read stayed empty, which is what separated the query defect from the genuine platform gap. Separately, an audit log script projecting initiators showed a wall of blank actors for events whose correlation IDs and timestamps proved they were real, because `$_.InitiatedBy.App` is a non-null object even when every field on it is null, so the condition evaluated true and the script read a null `DisplayName` instead of falling through to the populated `User` branch. Test a leaf field rather than a container's truthiness, and treat suspiciously uniform output as a reason to dump the raw response before treating it as a finding. The qualifier matters too: the `-Property` gap was not universal, working correctly against a live user addressed by id, which is what makes it a query defect rather than a general one.

**This lab got three things wrong about itself, and what caught every one of them was reading live state instead of trusting a state the document had already recorded.** Step Eight's Final State was accurate when it was written and inaccurate within a day, because `admin@brindeck.com` removed `John Smith` from `Company Announcements` at 4:03:32 PM on 9/7 after the step had closed, taking his license with him. Nothing noticed until Step Ten opened with its own baseline read and found the tenant no longer matched the document. Establishing what had happened then cost a full week's audit pull filtered client-side on object IDs, because Microsoft Entra records "Remove member from group" against the group rather than the user, so the portal's target-name filter on `John Smith` returned nothing at all. Separately, the "6 of 25" figure in that same Final State was recorded before anyone understood what the page was counting; it was never a wrong reading, it was an unexamined unit, and it took until Part C two steps later to establish the basis and append a dated correction rather than write one into a narrative that had been accurate at the time. The third sits at the opposite end of the lab from the other two, and it is the same blindness read from the front. Step One's baseline read the Users blade, the Groups blade, and `AZUREADSSOACC`, and never opened Deleted users or Deleted groups at all. Three objects were already sitting in those containers before the lab started, `duptest01` and `duptest02` from Lab 02 and `Testgroup` from before it, and each had to be discovered mid-lab and accounted for retroactively: the first two by Nine-A, which found Lab 02's record of a clean cleanup untrue in the tenant, and the third by Ten-B, which met it on the Deleted groups page while running an unrelated test. A baseline drawn only from the live population is blind to precisely the state a delete-and-restore lab exists to examine, and the counts it produces will reconcile perfectly while saying nothing about it, which is the same criticism Nine-A correctly levels at Lab 02's own reconciliation. All three cost the same thing. A number or a state recorded without being interrogated becomes something the document asserts, and later steps build on it. The habit the lab arrived at, and should have opened with, is that a figure read off a screen is not a finding until its unit is known, that a Final State is a claim about one moment rather than a standing description of the tenant, and that a baseline should enumerate the containers the lab's own subject matter can hide in rather than only the ones its headline counts come from.

---

## Sources

Every link below was checked as resolving at the close of this lab rather than reconstructed from the planning notes. The per-line list prices this lab records were read from the Microsoft 365 business plans pricing page and from the tenant's own Plans and pricing view at Step Two; only the pricing update announcement below carries a stable public URL, so the rest are attributed to the observation rather than to a link.

**Licensing, subscription, and pricing**
...
- [Microsoft 365 for business security overview](https://learn.microsoft.com/microsoft-365/admin/security-and-compliance/m365b-security-overview) - the per-tier table showing Business Basic and Standard on Entra ID Free and Business Premium on Entra ID P1 with Intune Plan 1, which is the specific gap this lab's trial closed
- [Try or buy a Microsoft 365 for business subscription](https://learn.microsoft.com/microsoft-365/commerce/try-or-buy-microsoft-365) - starting a trial from inside an existing tenant through the admin center so it joins the same organization; that all trial subscriptions include 25 free licenses for the trial period; and that turning off recurring billing makes a trial expire at the end of its month without charge instead of converting. The basis for Steps Two and Three, and confirmed against the tenant in both
- [Product names and service plan identifiers for licensing](https://learn.microsoft.com/entra/identity/users/licensing-service-plan-reference) - the service plan names and identifiers Step Four checked the subscription against by name, rather than inferring `AAD_PREMIUM`, `EXCHANGE_S_STANDARD`, and `INTUNE_A` from their resemblance to their friendly names
- [Get-MgSubscribedSku](https://learn.microsoft.com/powershell/module/microsoft.graph.identity.directorymanagement/get-mgsubscribedsku) - the cmdlet Step Four used to read a SKU's service plan list directly regardless of assignment state, which is the capability neither admin center's UI exposes, and whose `ConsumedUnits` figure Step Ten's Part C reconciled against the product page
- [Microsoft 365 Pricing and Packaging Updates](https://www.microsoft.com/en-us/licensing/news/2026-m365-packaging-pricing-updates) - the pricing change effective 2026-07-01, including Business Premium unchanged at $22.00 with Teams and $18.79 without and Business Basic (no Teams) moving from $4.40 to $5.40 on annual commitment. The basis for Design Decisions treating the recorded Business Basic figure as a post-increase monthly price rather than a stale one
...
- [Manage rules for dynamic membership groups in Microsoft Entra ID](https://learn.microsoft.com/entra/identity/users/groups-dynamic-membership) - the Microsoft Entra ID P1 requirement, stated per unique member rather than per assigned license; the security consideration about write permissions on attributes synchronized from Active Directory, including SELF write, which Step Six checked with `dsacls` and found not realized on this schema; and the constraints that membership cannot be edited by hand and that a rule cannot mix users and devices

**Licenses on users and groups**

- [Assign or unassign licenses to a group in the Microsoft 365 admin center](https://learn.microsoft.com/microsoft-365/admin/manage/manage-group-licenses) - group-based licensing as it is administered today, in the Microsoft 365 admin center rather than the Entra admin center; that nested groups are not processed; the twenty-group limit, which the Assign licenses panel's own copy corroborated in Step Eight; the documented order for moving a user between licensed groups and the service interruption reversing it causes, which Eight-D demonstrated and Eight-C produced by accident; and the Errors and issues tab Step Eight read. This page states no license prerequisite of its own, a claim Step Eight recorded against observation rather than repeating
- [Assign or unassign licenses for users in the Microsoft 365 admin center](https://learn.microsoft.com/microsoft-365/admin/manage/assign-licenses-to-users) - the per-user Licenses and apps path and the Active users bulk Manage product licenses path Step Five used; that License Administrator or User Administrator is the minimum role, which Step Seven tested live; the Errors and Issues categories Step Eight tried and failed to provoke, namely no available licenses, conflicting service plans, and an invalid usage location; and the statement that a group-assigned license shows the group's name on the Licenses page instead of the individual users, which is the documented basis for the target-versus-seat counting Part C established
- [Group-based licensing PowerShell examples](https://learn.microsoft.com/entra/identity/users/licensing-powershell-graph-examples) - that a group's own `AssignedLicenses` property is the documented way to read the product licenses assigned to it, which is the property Step Ten found reading empty on two groups the platform simultaneously refused to delete for carrying active licenses
...
- [Microsoft Entra Connect Sync service features](https://learn.microsoft.com/entra/identity/hybrid/connect/how-to-connect-syncservice-features) - the current `Get-MgDirectoryOnPremiseSynchronization` cmdlet and its `Features` list, and the `SynchronizeUpnForManagedUsers` feature that allows the sync engine to update `userPrincipalName` for managed, nonfederated users. The explanation Step Eleven-B read the tenant's own `True` value against rather than leaving the permitted UPN change as a bare surprise
- [How to use the BypassDirSyncOverridesEnabled feature of a Microsoft Entra tenant](https://learn.microsoft.com/entra/identity/hybrid/connect/how-to-bypassdirsyncoverrides) - that synchronized users' properties cannot be changed from the Entra or Microsoft 365 admin portals or through any PowerShell module, and that the historical `mobile` and `otherMobile` exception no longer applies. The basis for Step Eleven's attribute test, and consistent with the tenant reading `BypassDirSyncOverridesEnabled: False`
...
- [Microsoft Entra Connect Sync: Scheduler](https://learn.microsoft.com/entra/identity/hybrid/connect/how-to-connect-sync-feature-scheduler) - the thirty-minute default cycle, delta versus full, and the `Get-ADSyncScheduler` properties Steps Six, Nine, Eleven, and Twelve each read before relying on a cycle. Step Twelve is where those properties reported a healthy configuration over a timer that had not fired in two days

**Roles**

- [Use Microsoft Entra groups to manage role assignments](https://learn.microsoft.com/entra/identity/role-based-access-control/groups-concept) - that role-assignable groups require Microsoft Entra ID P1 and must use assigned rather than dynamic membership, that Privileged Identity Management requires P2, and that Microsoft Entra roles cannot be assigned to on-premises groups. Step Seven confirmed the assigned-membership constraint is enforced by the product locking the field rather than by rejecting a change
- [Assign Microsoft Entra roles](https://learn.microsoft.com/entra/identity/role-based-access-control/manage-roles-portal) - assignment at tenant scope, and the scoped alternatives this lab did not use
- [Least privileged roles by task in Microsoft Entra ID](https://learn.microsoft.com/entra/identity/role-based-access-control/delegate-by-task) - the least-privileged role per administrative task, including License Administrator for license assignment and User Administrator for restoring deleted users. The basis for Step Seven starting below Global Administrator, and for reading License Administrator's documented permissions against the group creation that succeeded for an unrelated reason

**Deletion and recovery**

- [Restore or remove a recently deleted user](https://learn.microsoft.com/entra/fundamentals/users-restore) - the thirty-day soft-delete window and what it preserves; that Microsoft Entra is not the source of authority for synchronized users and the synchronization engine may restore one that still exists on-premises, which Step Nine observed happening on a scheduled cycle; that a user deleted more than thirty days ago cannot be restored by anyone including Microsoft support; and that restoring a user restores its licenses even when none are available
- [Recover from deletions](https://learn.microsoft.com/entra/architecture/recover-from-deletions) - the properties each object type retains through soft delete, and the statement that soft-deleted Microsoft 365 groups and cloud security groups both appear on the Deleted groups page and can be restored. This is the guidance that held against the tenant in Step Ten
- [Recover deleted data (Microsoft Entra PowerShell)](https://learn.microsoft.com/powershell/entra-powershell/recover-deleted-data) - the conflicting statement that only Microsoft 365 groups can be restored and that security groups cannot. Still stated as of this lab's close, and contradicted by the tenant in Step Ten, which is why the contradiction was settled by test rather than by citing either page
...
- [Remediate hybrid security posture assessments in Defender for Identity](https://learn.microsoft.com/defender-for-identity/security-posture-assessments/hybrid-security) - the Change password for Microsoft Entra seamless SSO account assessment, which lists seamless SSO computer accounts whose password was last set over ninety days ago rather than thirty. The independent corroboration Step Twelve used to characterize the thirty-day recommendation as an interval rather than a deadline
