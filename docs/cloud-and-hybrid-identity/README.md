...
That trial no longer converts. Recurring billing on it has been disabled, so it lapses on 2026-09-22 instead, and the decision the track owed on it is resolved: the Basic trial is not kept, and what replaces it is a trial rather than a purchase.

The replacement arrives one lab earlier than planned. Lab 03 needs Microsoft Entra ID P1 for dynamic membership groups and role-assignable groups, both of which are in its stated scope, so the licensing arrival [ADR-019](../architecture/decisions/019-establish-cloud-and-hybrid-identity-track.md) Design Decision 2 anticipated falls at Lab 03 rather than Lab 04. A Microsoft 365 Business Premium thirty-day trial is started from inside the existing tenant as a documented step of Lab 03, carrying Microsoft Entra ID P1 for that lab, Exchange Online for Lab 04, and Microsoft Intune and conditional access for Lab 05 in a single subscription.
...
Two consequences follow and are handled in the labs rather than assumed away. Recurring billing is turned off on the new trial the day it starts, because a trial converts to a paid subscription at the end of its period unless it is, which is a documented default and a real trap. And Lab 03's steps are ordered so the P1-dependent work lands early in the thirty days, ahead of the work that behaves identically on any tier. What to do when the window closes is a decision Labs 04 and 05 each take for themselves.

Acquiring Microsoft Entra ID P1 at Lab 03 does not move conditional access earlier. ADR-019 Design Decision 6 keeps the security defaults to conditional access transition, and the obligation to reproduce the protections being removed rather than dropping them silently, with Lab 05. Lab 03 uses P1's group and licensing capabilities only, and leaves security defaults enabled.
...
| [03 - Entra ID User, Group, and License Administration](03-entra-id-user-group-and-license-administration.md) | Planning and research |
