...
Steps One through Six-B are complete: implemented, run against the live environment, and documented in past tense below. Step Seven (independent validation, monitoring-stack remediation, and the closing library sweep) remains planning and research, written in future tense. The scheduled task is registered on WIN11-CLIENT01 and has fired unattended, confirmed by a genuine time-trigger firing rather than a manual run; no combined analyzer and Pester sweep across Labs 01 through 05 has been run.
...
**Decision:** Each check script will classify its result as `Healthy`, `Unhealthy`, or `Unknown`. `Unknown` is reserved specifically for "the check itself could not be completed" (DC01 unreachable, a Wazuh or Portainer authentication failure, a request timeout), distinct from `Unhealthy`, which means the check completed and found a real problem (a stopped service, a disconnected agent, a stopped container). Implementation later added a third shape to that first category, found live rather than anticipated here: a query that succeeds, throws nothing, and returns nothing, which is a failed observation wearing a success code and belongs with the other `Unknown` cases rather than with the real problems (see Step Six-A and Troubleshooting and Adjustments). `Invoke-LabHealthReport.ps1` aggregates the three per-check results into one overall status using a worst-wins rule: any `Unhealthy` check makes the overall status `Unhealthy`, regardless of the other two; failing that, any `Unknown` check makes the overall status `Unknown`; only if all three checks report `Healthy` is the overall status `Healthy`.
...
Whether Task Scheduler's non-interactive execution context changes the orchestrator's behavior was genuinely untested at the planning stage, and it is why Step Six-B required observing a real scheduled firing rather than a manually triggered one. The relevant script is `Invoke-LabHealthReport.ps1` specifically, not the three check scripts, since a scheduled firing invokes only the orchestrator and reaches the checks as dot-sourced functions inside it. The surfaces actually at risk are its interactive guard's `Get-Credential` and `Read-Host` calls, addressed in Step Six-A; `$MyInvocation.InvocationName` resolving to the script path rather than `.` under `powershell.exe -File`, which the whole run depends on; `$PSScriptRoot` resolving so the three dot-sourced check scripts are found; the calling account's execution policy; `Get-LabWazuhAgentStatus.ps1`'s runtime `Add-Type` compilation, which needs a writable temporary directory; and whether the report directory can be created and written under the task's own token. None of the four scripts uses `Write-Host`, so console-narration output is not among the risks, despite being the obvious first guess for a non-interactive context.

**Confirmed by Implementation Step Six-B: the non-interactive context introduced no behavioral difference at all.** The observed firing completed with `LastTaskResult 0` and wrote its timestamped report, which is only possible if every surface listed above behaved: the guard reached no prompt, `$MyInvocation.InvocationName` resolved to the script path so the guard ran, `$PSScriptRoot` resolved so the three check scripts were dot-sourced, the execution policy allowed the file to run, `Add-Type` compiled the certificate-policy class under the task's own token, and the report directory was writable. The report's contents matched what a manual run of the same scripts returns. The two real surprises this step produced were not in the orchestrator at all but in `Set-ScheduledTask`, which turned out to require the run-as credential to be re-supplied on every modification of a `LogonType Password` task and, separately, an elevated session; both are recorded in Troubleshooting and Adjustments. This closes the last open item in this Design Decision.
...
**That rejected account left behind a real defect in a script this step was not meant to touch.** The empty container list was recorded above as a platform finding and nothing more, until a review asked what `Get-LabDockerServiceStatus.ps1` would actually have done with that exact response. The answer is that it would have walked its eight-entry `-ExpectedContainer` list, matched none of them, reported eight `NotFound` rows, and classified the check `Unhealthy` with a blank `Message`: a full-environment outage reported for a query that had in fact succeeded and simply could not see anything. That is the same misclassification Step Two found and fixed in `Get-LabADServiceHealth.ps1`, arriving by a different route. There, `-Name` with `-ErrorAction SilentlyContinue` made an unreachable host indistinguishable from absent services; here, a permissions-blinded API response is indistinguishable from every container having vanished. Both collapse a failed observation into a confirmed fault, which is precisely the direction Design Decision 4 introduced `Unknown` to avoid, and both were missed by test suites that were fully passing at the time.
...
`Invoke-LabHealthReport.ps1` then gained a `Get-LabStoredCredential` helper (an `Import-CliXml` wrapper, validating explicitly: a clear throw if the file is absent, a different clear throw if it does not deserialize to a `[PSCredential]`) and an optional top-level parameter wiring it into the guard for whichever credential was not supplied directly, preferred over the `Get-Credential` prompt. The parameter was originally planned as `-CredentialDirectory`, but PSScriptAnalyzer's `PSAvoidUsingPlainTextForPassword` flagged it: the rule's own word list is `Password`, `Passphrase`, `Cred`, `Credential`, matched case-insensitively against any `[string]`-typed parameter, so no name containing "Cred" or "Credential" clears it. It was renamed to `-SecretsDirectory`, consistent with how this track has always resolved analyzer findings, by fixing the code, not suppressing the rule. `Invoke-LabHealthReport`'s own three `Mandatory` parameters and body were untouched. Three Contexts were added to `Invoke-LabHealthReport.Tests.ps1`: a genuine round trip through `Export-CliXml`/`Import-CliXml` in `TestDrive:\`, a missing-file throw, and a wrong-type throw. The full suite, thirty-eight original tests plus three new ones, passed 41 of 41, and both files returned a clean `Invoke-ScriptAnalyzer` pass.
...
**Step Six-B: built `Register-LabHealthReportTask.ps1`, registered the task, and observed a real firing.** Built `Register-LabHealthReportTask.ps1` taking a `[PSCredential]` for the run-as account rather than separate username and password parameters, both because that is the discipline every credential in this lab already follows and because a `Password` parameter would trip `PSAvoidUsingUserNameAndPasswordParams` and `PSAvoidUsingPlainTextForPassword`; the plaintext password `Register-ScheduledTask` requires is unwrapped at the call site only, via `$RunAsCredential.GetNetworkCredential().Password`, and held in no variable of its own. A live `Get-Help Register-ScheduledTask -Full`, checked rather than assumed, showed four parameter sets and confirmed that the one built around `-Principal` carries no `-Password` parameter at all; only the `Xml`, `User`, and `Object` sets accept a plaintext password. The script therefore registers through the `User` parameter set: `-User`, `-Password`, and `-RunLevel Limited` passed directly to `Register-ScheduledTask`, never `New-ScheduledTaskPrincipal` or `-Principal`. `Register-` is on PSScriptAnalyzer's state-changing-verb list, so this function implements `[CmdletBinding(SupportsShouldProcess = $true)]` for real, gating the actual `Register-ScheduledTask` call behind `$PSCmdlet.ShouldProcess(...)`, rather than renaming its way around the rule as Step Five's `ConvertTo-LabHealthReportHtml` did; it is the first script in the track to do so, since it is the first one that actually changes state. Re-running the script against an existing task name was given a defined behavior rather than left to fail obscurely: `Get-ScheduledTask` is queried first, unconditionally, and the function throws naming the task and directing the caller to `-Force` unless `-Force` was supplied.
...
Building the Pester suite, which mocks the `ScheduledTasks` cmdlets per the Lab 03 standard so the registration logic is verified without registering anything, surfaced three real findings rather than confirming the design on the first attempt. The first Pester run, four of fifteen tests passing, failed with `Cannot convert the ... PSCustomObject to type Microsoft.Management.Infrastructure.CimInstance[]`: Pester's `Mock` enforces a mocked cmdlet's own real parameter type metadata, so a hand-built `PSCustomObject` standing in for `New-ScheduledTaskAction`'s or `New-ScheduledTaskTrigger`'s output failed to bind to `Register-ScheduledTask`'s `CimInstance[]`-typed `-Action` and `-Trigger` parameters even though both cmdlets were mocked. Fixed by leaving `New-ScheduledTaskAction`, `New-ScheduledTaskTrigger`, and `New-ScheduledTaskSettingsSet` unmocked, all three being side-effect-free, and asserting on their real `CimInstance` output as received by the still-mocked `Register-ScheduledTask`. The second run, thirteen of fifteen passing, failed with `Cannot validate argument on parameter 'Password'. The argument is null or empty`: `Register-ScheduledTask`'s real `-Password` parameter rejects an empty string even under Mock, so the test credential needed a non-empty `SecureString`, built with `.AppendChar()` rather than `ConvertTo-SecureString -AsPlainText`, which would have tripped `PSAvoidUsingConvertToSecureStringWithPlainText`. The third run left two trigger-matching tests still failing on "called 0 times"; rather than guess again, the actual `StartBoundary` value was captured live (`(New-ScheduledTaskTrigger -Daily -At '07:00').StartBoundary`, returning `2026-08-19T11:00:00Z`), which showed the trigger's `StartBoundary` is UTC and `Z`-suffixed rather than a local-time string. The trigger assertions were rewritten to parse it through `[datetimeoffset]$PesterBoundParameters['Trigger'][0].StartBoundary).LocalDateTime`, robust to the UTC/local distinction rather than assuming a fixed offset. The suite reached fifteen of fifteen tests passing, and both `Register-LabHealthReportTask.ps1` and its test file were analyzer-clean throughout, with no findings on any pass.
...
With the trigger moved forward, the operator logged off entirely rather than merely locking the session, since "run whether the user is logged on or not" is the configuration actually under test, and waited past the new trigger time before logging back in. `Get-ScheduledTaskInfo` afterward showed `LastRunTime` at the moved trigger time and `LastTaskResult 0`, and `Get-ChildItem C:\Reports | Sort-Object LastWriteTime -Descending` showed a report file timestamped to that same moment and distinct from any earlier manual run.
...
The fix in each script is a guard between the `try`/`catch` and the matching loop, returning `Unknown` with an explanatory `Message` when the response contains nothing. An empty list is unambiguous in this environment rather than a possible true reading: the `portainer` container is in the Docker check's own expected set and Portainer answered the request, and agent `000` is always present in a working Manager's agent list, so in both cases a response containing nothing cannot describe the system that just served it. The Wazuh guard deliberately tests the raw response before agent `000` is filtered out, so that a response carrying only agent `000`, which is a real "no monitored agents enrolled" state, still classifies `Unhealthy` rather than being masked. Both guards filter nulls, covering an empty body as well as an empty array, without altering the assign-then-wrap form Step Four's array-nesting fix depends on. Four regression tests were added, two per script, taking `Get-LabDockerServiceStatus.Tests.ps1` from seventeen tests to nineteen and `Get-LabWazuhAgentStatus.Tests.ps1` from fifteen to seventeen, all passing, with all four analyzer invocations clean. The defect was latent rather than live, since the scheduled path uses the Portainer admin account, which returns the full list.
...
- **The Portainer API path is HTTP-only, so its credential crosses the LAN in cleartext on every call, and it remains the broad admin account.** Implementation Step One confirmed the only working Portainer path is `http://portainer.local` through NGINX Proxy Manager, not HTTPS; the proxy host has no SSL certificate assigned. `Get-LabDockerServiceStatus.ps1`, built in Step Four, therefore sends Portainer's admin credential over plain HTTP internally on every invocation, confirmed directly by Step Four's own live runs and again by Step Six-A's. Step Six-A tried to replace it with a dedicated, least-privileged, read-only account and found Portainer Community Edition does not support one: a non-admin user cannot list existing containers regardless of endpoint access, by the platform's own design (see Design Decision 5). This is a named, accepted exposure rather than a solved one: a compromised or misconfigured segment of the LAN could observe the admin credential in transit.
...
At the planning stage, the expected outcome is five PowerShell scripts, three focused health checks, one orchestrator, and the script that registers it as a scheduled task, giving the environment a single, on-demand or scheduled answer to "is everything currently healthy," where today that answer requires checking DC01's services, the Wazuh dashboard, and Ubuntu Server's Docker state as three separate manual steps. The lab is expected to close the track's final stated success criterion and, with it, the Infrastructure Automation and Scripting track itself, per ADR-018.

---

## Lessons Learned

Lessons cannot honestly be recorded for the lab as a whole before the monitoring-stack outage is remediated and re-validated alongside the full combined script-library sweep (Step Seven); this section will be completed once that work is done. Step Six's own half of that condition is now met: the scheduled task is registered and has been observed firing unattended on a real time trigger. All five scripts' own findings, through Step Six-B, are recorded where they belong, in each step's Implementation and in Troubleshooting and Adjustments above, rather than anticipated or duplicated here. Both of Design Decision 5's open items are now closed, the run-as account by the Service Control Manager probe and the non-interactive execution context by the observed firing. The question most likely to produce further lessons is the one Step Seven still owes: whether the before/after comparison it plans, `Unhealthy` before remediation and `Healthy` after, actually demonstrates worst-wins discriminating a real fault from a clean environment the way Design Decision 4 intends.
